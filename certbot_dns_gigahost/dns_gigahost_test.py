"""Tests for certbot_dns_gigahost.dns_gigahost."""
...
from certbot_dns_gigahost.dns_gigahost import Authenticator, GigahostClient
...
class TestAuthenticator(
    ...
    """
    Test for gigahost DNS Authenticator
    """
    ...
    def setUp(self):
        ...
        dns_test_common.write(
            {
                "gigahost_account_name": "account_name",
                "gigahost_api_key": "api_key",
            },
            path,
        )
        ...
        self.config = mock.MagicMock(
            gigahost_credentials=path, gigahost_propagation_seconds=0
        )

        self.auth = Authenticator(config=self.config, name="gigahost")
        ...
        # _get_gigahost_client | pylint: disable=protected-access
        self.auth._get_gigahost_client = mock.MagicMock(return_value=mock_client_wrapper)
...
class TestgigahostClient(unittest.TestCase):
    """
    Test for gigahost API Client
    """

    def setUp(self):
        self.client = GigahostClient("username", "password")
    ...
    def test_add_txt_record(self, request_mock):
        request_mock.post(
            f"https://api.gigahost.dk/2/my/products/{self.object_id}/dns/records/",
            status_code=200,
            json=[{}],
        )
    ...
    def test_del_txt_record_fail(self, request_mock):
        ...
        self._my_products_get_mock(request_mock, self.object_id)
        request_mock.delete(
            f"https://api.gigahost.dk/2/my/products/{self.object_id}/dns/records/123/",
            status_code=400,
            text="Error",
        )
    ...
    def _my_products_get_mock(
        ...
        request_mock.get(
            "https://api.gigahost.dk/2/my/products/",
            status_code=200,
            json={
                "products": [
                    {
